...
p_obj_str = r'\[\w+,*\s*\w*\]\s*\w+\**\s*\w+'
m_obj_str = r'\[id\(\d+\)\s*,\s*helpstring\("(?:[a-zA-Z0-9_\u4e00-\u9fa5]+\s*)+"\)\]\s*HRESULT\s*\w+\((?:' + p_obj_str + r'\s*\,*\s*)+\);'
a_obj_str = r'\[\s*object,\s*uuid\([a-zA-Z0-9\-]+\),\s*dual,\s*nonextensible,\s*helpstring\("(?:[a-zA-Z0-9_\u4e00-\u9fa5]+\s*)+"\),\s*pointer_default\(unique\)\s*\]'
i_obj_str = a_obj_str + r'\s*interface\s*\w+\s*:\s*\w+\s*\{\s*(?:' + m_obj_str + r'\s*)*\};'
...
p_obj_str_g = r'\[(?P<p_io_type>\w+,*\s*\w*)\]\s*(?P<p_type>\w+\**)\s*(?P<p_name>\w+)'
m_obj_str_g = r'\[id\((?P<m_id>\d+)\)\s*,\s*helpstring\("(?P<m_helpstr>(?:[a-zA-Z0-9_\u4e00-\u9fa5]+\s*)+)"\)\]\s*HRESULT\s*(?P<m_name>\w+)\((?P<params>' + p_obj_str + r'\s*\,*\s*)+\);'
a_obj_str_g = r'\[object,\s?uuid\((?P<a_uuid>[a-zA-Z0-9\-]+)\),\s?dual,\s?nonextensible,\s?helpstring\("(?P<a_helpstr>(?:[a-zA-Z0-9_\u4e00-\u9fa5]+\s*))"\),\s?pointer_default\(unique\)\s*\]'
i_obj_str_g = a_obj_str + r'\s*interface\s*(?P<i_name>\w+)\s*:\s*(?P<i_parent>\w+)\s*\{\s*(?:' + m_obj_str + r'\s*)*\};' 
